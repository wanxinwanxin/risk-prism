...
def build_model_md(artifacts_dir: str | Path) -> str:
    ...
    lines += [
        ...
        *_validation_md(a.get("validation")),
        ...
        f"   ×{int(cfg.get('ann_factor', 52))}, repaired to PSD, then scaled by the factor Volatility",
        f"   Regime Adjustment multiplier (this build: {m.get('vra_factor', 1.0)}) — an EWMA",
        f"   ({cfg.get('vra_half_life', 8)}w half-life) of the cross-sectional bias statistic, so the",
...
POST /api/v1/portfolio-risk with {{"weights": {{"AAPL": 0.4, "MSFT": 0.4,
"XOM": 0.2}}}} returns annualized vol with factor/specific decomposition.
Also: GET /api/v1/meta, /api/v1/factors, /api/v1/assets/{{ticker}},
/api/v1/coverage?tickers=..., POST /api/v1/stress-test.
"""
